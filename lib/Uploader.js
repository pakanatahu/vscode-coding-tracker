...
// no vscode API usage needed directly here
...
const DESKTOP_DISCOVERY_KEY = 'codingTracker.desktopDiscovery';
const DESKTOP_TOKEN_KEY = 'codingTracker.desktopToken';
const TRUSTED_SOURCE_KEY = 'codingTracker.trustedSource.v1';
const TRACKING_CONFIG_KEY = 'codingTracker.trackingConfig.v1';
...
const CLOUD_ENDPOINT_CANDIDATES = ['api/upload', 'api/queue/upload', 'ajax/upload', 'upload', 'api/track', 'track'];
...
let endpointCandidates = DESKTOP_ENDPOINT_CANDIDATES.slice();
...
/** @type {{ appendMany:(events: UploadPayload[]) => Promise<void> }|null} */
	...
	try {
		...
		if (Array.isArray(parsed)) {
			...
			if (!queueWarned && Q.length > QUEUE_WARN_THRESHOLD) {
				queueWarned = true;
				try { vscode.window.showWarningMessage(`CodingTracker: Offline queue has ${Q.length} pending events. Desktop app may be offline.`); } catch(_) { /* ignore */ }
			}
		}
...
const uploader = {
	...
	setConnectionMode: function(mode) {
		if (mode === 'desktop' || mode === 'cloud') {
			...
			try { log.debug(`uploader connectionMode set to ${mode}`); } catch(_) { /* ignore */ }
		}
	},
...
async function _upload() {
	...
	// Set up upload token.
	// In desktop local-only mode (requiresToken === false), omit token entirely.
	...
	if (isDesktopUploadEndpoint) {
		// Trusted Upload v1 contract: send JSON body with events[].
		...
		// Non-desktop API endpoints keep JSON payload semantics.
	...
	// Inject dynamic headers (Origin + function key) each request (in case settings changed)
	try {
	...
	} catch(e) { try { log.debug('Header injection failed: ' + JSON.stringify(e)); } catch(inner) { /* ignore */ } }

	// Proxy handling: if explicit string URL provided, use proxy-agent; if 'no-proxy' or false, disable proxy
	if (typeof uploadProxy !== 'undefined') {
		if (uploadProxy === false || uploadProxy === 'no-proxy') {
			...
			// Basic proxy support via axios proxy option when a host:port is provided
			try {
				const u = new URL(uploadProxy);
				uploadOptions.proxy = { host: u.hostname, port: Number(u.port || (u.protocol === 'https:' ? 443 : 80)) };
			} catch(_) {
				// If not a URL, try host:port pattern
...
async function ensureDesktopReady() {
	const now = Date.now();
	// Cloud mode: skip desktop discovery entirely and use configured base URL.
	if (connectionMode === 'cloud') {
		...
		uploadURL = baseServerURL + endpointCandidates[currentEndpointIndex];
		// Cloud ingest uses the legacy upload token only; no local token handshake.
		return true;
	}
...
/**
 * Keep desktop LocalApi base at host root so request URLs become /api/... exactly once.
 * @param {string} url
...
function clampDurationMs(value) {
	...
	if (!Number.isFinite(numeric) || numeric <= 0) return MIN_DURATION_MS;
...
async function refreshToken(token) {
	...
	try {
		...
		if (res && res.status === 404) {
			...
			try { vscode.window.showWarningMessage('CodingTracker: Desktop token expired. Requesting a new token...'); } catch(_) { /* ignore */ }
			return 'invalid';
		}
...
async function requestNewToken() {
	const base = baseServerURL || fallbackBaseServerURL;
	if (!base) return !!uploadToken; // fallback to legacy token if present
	...
	return !!uploadToken; // allow legacy token fallback
}
...
async function handleTokenFailure(message) {
	try { log.debug('Auth failure, clearing token:', message); } catch(_) { /* ignore */ }
	await clearToken('auth-failed');
	try { vscode.window.showWarningMessage('CodingTracker: Desktop token invalid or expired. Requesting a new one...'); } catch(_) { /* ignore */ }
