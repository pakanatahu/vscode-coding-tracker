...
const uploader = {
	...
	setForceLocalFallback: function(enabled) {
		...
		try { log.debug(`uploader forceLocalFallback set to ${forceLocalFallback}`); } catch (_) { /* ignore */ }
	},
...
async function handleTokenFailure(message) {
	...
	try { vscode.window.showWarningMessage('SlashCoded: Desktop token invalid or expired. Requesting a new one...'); } catch(_) { /* ignore */ }
