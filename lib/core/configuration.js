...
async function updateConfigurations(deps) {
    ...
    try {
        if (activationContext && activationContext.secrets) {
            ...
            if (secret && secret.trim()) {
                ...
                try { vscode.window.showInformationMessage('SlashCoded: Upload token migrated to secure storage.'); } catch (_) { /* ignore */ }
            }
        } else {
            uploadToken = uploadTokenCfg;
        }
    ...
    try { uploader.setForceLocalFallback(forceLocalFallback); } catch (e) { log.debug('Failed to set forceLocalFallback on uploader', e); }
