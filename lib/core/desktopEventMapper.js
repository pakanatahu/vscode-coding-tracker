...
function getProducerTimezoneMetadata(atMs = Date.now()) {
    let timezone = null;
    try {
        const resolved = Intl.DateTimeFormat().resolvedOptions();
        timezone = typeof resolved.timeZone === 'string' && resolved.timeZone ? resolved.timeZone : null;
    } catch (_) {
        timezone = null;
    }

    const date = new Date(Number(atMs) || Date.now());
...
function mapToDesktopEvent(src, trackingConfig) {
    ...
    const timezone = getProducerTimezoneMetadata(segmentEndTs);
