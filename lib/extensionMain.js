...
async function activate(context) {
    ...
    try { outLog.debug('CodingTracker: activating extension...'); } catch (_) { /* ignore */ }
        ...
        if (pick.action === 'queue-local-history') {
            try {
                ...
                vscode.window.showInformationMessage(importedCount > 0
                    ? `CodingTracker: queued ${importedCount} local event${importedCount === 1 ? '' : 's'} for Desktop ingestion.`
                    : 'CodingTracker: no local-only history found to queue.');
            } catch (e) { log.error(e); }
